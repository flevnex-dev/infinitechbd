...
class db_class {
    ...
    public function open() {
        $con = mysqli_connect("localhost", "root", "", "infinitechbd");
        return $con;
    }

    public function pdocon() {
        $db = new PDO("mysql:host=localhost;dbname=infinitechbd;", "root", "");
    ...
    function baseUrl($suffix = '') {
        ...
        $web_root = $protocol . $_SERVER['HTTP_HOST'] . "/" . "a.m.softltd/Banglar karuka/new/INFINITECH/cms-admin/";
        $suffix = ltrim($suffix, '/');
    ...
    function LbaseUrl($suffix = '') {
        $protocol = strpos($_SERVER['SERVER_SIGNATURE'], '443') !== false ? 'https://' : 'http://';
        $web_root = $protocol . $_SERVER['HTTP_HOST'] . "/" . "a.m.softltd/Banglar karuka/new/INFINITECH/";
        $suffix = ltrim($suffix, '/');
        return $web_root . trim($suffix);
    }
